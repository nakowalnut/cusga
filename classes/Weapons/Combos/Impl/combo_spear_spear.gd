...
func execute(player: CharacterBase, current_weapon: WeaponBase, next_weapon_name: String) -> void:
	...
	for enemy in enemies:
		...
		var original_attack_cd = 0.0
		if "attack_controller" in enemy and is_instance_valid(enemy.attack_controller):
			original_attack_cd = enemy.attack_controller.default_cooldown
		affected_targets.append({
			"target": enemy,
			"speed": original_speed,
			"attack_cooldown": original_attack_cd
		})

		if "speed" in enemy:
			enemy.speed = original_speed * 0.7
		if "attack_controller" in enemy and is_instance_valid(enemy.attack_controller):
			enemy.attack_controller.default_cooldown = original_attack_cd * 1.3
	...
	for item in affected_targets:
		...
		if "attack_controller" in target and is_instance_valid(target.attack_controller):
			target.attack_controller.default_cooldown = item["attack_cooldown"]
